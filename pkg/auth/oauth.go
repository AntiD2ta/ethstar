...
func SetNamedStateCookie(w http.ResponseWriter, name, state string, secure bool) {
	// #nosec G124 -- Secure is driven by env (ETHSTAR_COOKIE_SECURE); must be false on plaintext localhost dev since browsers reject Secure cookies over http://. HttpOnly and SameSite are set.
...
func ClearNamedStateCookie(w http.ResponseWriter, name string, secure bool) {
	// #nosec G124 -- Secure must match the flag used at set-time (env-driven); false on localhost dev. HttpOnly and SameSite are set.
	http.SetCookie(w, &http.Cookie{
		Name:     name,
		Value:    "",
		Path:     "/",

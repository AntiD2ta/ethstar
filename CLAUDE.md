...
- **CLS prevention with `invisible` breaks centered flex rows**: Using `invisible` to reserve space for a conditional stats column causes the row to appear off-center — the invisible element takes layout space on one side, pushing visible items away from center. With `justify-center` flex rows, invisible placeholders are counterproductive. Accept the minor CLS when data arrives rather than permanently breaking visual centering. The `invisible` technique only works for non-centered containers where the extra space isn't noticeable.
- **Font loading: CSS `@import` vs HTML `<link>`**: Moving Google Fonts from CSS `@import url(...)` to `<link>` in `index.html` eliminates 2 hops from the loading waterfall (HTML → Google CSS → font files vs HTML → app CSS → Google CSS → font files). Add `<link rel="preconnect">` for both `fonts.googleapis.com` and `fonts.gstatic.com` (with crossorigin on the latter) to establish connections during HTML parsing.
- **Use correctly-sized image assets**: A 2048x2048 PNG (1.5 MB) served for a 40px header icon wastes 99% of bandwidth. Reference the correctly-sized variant directly (`logo-128.png` for header, `logo-512.png` for hero fallback). Always add explicit `width`/`height` HTML attributes to prevent layout shift.
- **Suspense placeholder paradox**: Adding a visible loading placeholder (pulsing logo) to a `<Suspense>` boundary can make loading feel *slower* than `fallback={null}`, because it draws attention to the wait. On localhost the Three.js chunk loads in a fraction of a second — with `null` fallback the scene pops in seamlessly; with a placeholder users actively watch and wait. Prefer invisible fallbacks for fast-loading lazy components.

...
- **CLS prevention with `invisible` breaks mobile layout**: Using Tailwind `invisible` (visibility: hidden) to always render placeholder stats columns prevents CLS on desktop, but on mobile the invisible element still takes space and can overflow narrow viewports. Use responsive classes: `hidden md:block md:invisible` — `display:none` on mobile (no space), `visibility:hidden` on desktop (prevents CLS).

...
- **gosec G124 is cookie security (CWE-614), NOT file/dir permissions**: The rule name "http.Cookie missing or has insecure Secure, HttpOnly, or SameSite attribute" fires on any `http.SetCookie` where `Secure: false` is possible. Required suppression for env-driven cookies where `Secure` must be false on plaintext localhost dev (browsers reject Secure cookies over http://). Use `#nosec G124 -- CWE-614: ...` and ensure `HttpOnly` + `SameSite` are still set.
...
- **IntersectionObserver: observe the tall element, not a sentinel**: For "mount this UI only after the user scrolls past section X", observe X itself (e.g. a `min-h-dvh` hero section) via `React.forwardRef`/React 19 `ref` prop — do NOT place a zero-height sentinel `<div>` below it. On short viewports the sentinel is below the fold on initial render, so `isIntersecting=false` fires immediately and the UI mounts before any user scroll. Observing the section directly gives `isIntersecting=true` while any part is visible, flipping to `false` only when fully scrolled past. Regression test: assert the UI is hidden at viewport 1440×600 (shorter than hero content).
- **React 19 `ref` as a regular prop**: On React 19, `forwardRef` is unnecessary — declare `ref?: Ref<HTMLElement>` in the component's props interface, destructure it, and pass to the target element. Keeps the component signature flat and removes the wrapper ceremony.
- **`tw-animate-css` ships height-animating keyframes**: `@keyframes accordion-down/up` and `collapsible-down/up` animate `height: 0 → auto`. Even if your code never uses Radix Accordion/Collapsible, these keyframes sit in the shipped CSS bundle and trip static detectors that flag "`layout-transition: height` on all routes". Override with opacity-only equivalents in `index.css` after the `@import "tw-animate-css"`.
- **Pre-existing consent banner blocks role-based queries in E2E tests**: Phase C's Radix Dialog consent banner sets `aria-hidden` on sibling content on first load, which makes `page.getByRole(...)` fail across every home-page spec. Fix: add a `seedConsent(page)` helper in `e2e/helpers.ts` that writes a valid `ethstar_consent` localStorage entry via `page.addInitScript`, and invoke it from a top-level `test.beforeEach` in every affected spec file (home, responsive, loading-stats, manual-star-modal, resilience, saturn-carousel, tip-dialog).
- **Playwright CWD matters**: `npx playwright test` must run from `frontend/` (where `playwright.config.ts` lives). Running from the repo root silently discovers zero tests or produces `test.describe() called here` errors referencing unrelated files. Always `cd frontend` or use `-C frontend`/`--prefix frontend`.
...
- **`react-zoom-pan-pinch` `TransformWrapper` + `centerOnInit` fires `onZoom` synchronously**: If you wire `onZoom`/`onPanning` handlers that dismiss a UI hint, the hint is dismissed during the initial center calculation *before the user sees it*. Rely on a `setTimeout` auto-dismiss and only wire event handlers for genuine user gestures (or use `onPinchStart` instead).
- **`react-zoom-pan-pinch` v4 prop shapes**: `wheel?: { disabled?, wheelDisabled?, touchPadDisabled?, activationKeys? }`, `panning?: { disabled?, velocityDisabled?, lockAxisX?, lockAxisY?, ... }`, `doubleClick?: { disabled?, step?, mode?: "zoomIn" | "zoomOut" | "reset" | "toggle", ... }`, plus top-level `initialScale?`, `minScale?`, `maxScale?`, `centerOnInit?: boolean`. Exported names: `TransformWrapper`, `TransformComponent`.
- **Wrapper div inside `<TransformComponent>` must carry real dimensions**: The inner wrapper receives CSS `transform: translate(...) scale(...)` from react-zoom-pan-pinch. Pass `wrapperClass="!h-full !w-full"` and `contentClass="!h-full !w-full"` to force it to fill its parent (the `TransformWrapper` content area), otherwise it collapses to 0x0 and children are invisible.
- **Chrome MCP screenshot fails on small viewports with DOM-only content**: When the browser extension captures a tab with a narrow viewport (< ~400px) that has mostly absolutely-positioned content and no visible painted rects in the upper portion, screenshots come back black. Functional assertions (`elementFromPoint`, `getBoundingClientRect`, `getComputedStyle`) still work. Pivot to Playwright E2E with controlled viewports for reliable mobile validation.
- **`prefersReducedMotion || !isDesktop` prevented mobile animation**: When gating `useSaturnAnimation` with `!isDesktop`, the mobile ring never received `rAF` ticks and chips stayed at `transform: ""`. Remove `!isDesktop` from the disable predicate so the mobile ring animates too. The config reference stays stable because `isDesktop ? CONST_A : CONST_B` returns the same module-level object every render.
- **`useState(true)` + StrictMode double-mount still preserves hint visibility**: Each effect mount in StrictMode clears the previous timer and starts a new one, so the 4-second hint still shows fully. Don't add unnecessary guards against StrictMode for this pattern — it already works.
- **`react-zoom-pan-pinch` scale inside `perspective` still flattens 3D**: The `transform: scale(...)` applied by TransformComponent creates a 2D stacking context, so descendant `rotateX/Y` rotations appear flat even under an ancestor `perspective: 800px`. For mobile 3D rings inside a zoom wrapper, accept the flattened projection — chips still rotate around the Z axis correctly via `rotateZ`+`translateX` math.
- **Playwright `getByText` is case-insensitive substring match by default**: `dialog.getByText("Send an ETH Tip")` matches both a `<h2>` title and any `sr-only <p>` description containing "send an ETH tip" (different case). For dialogs, prefer `getByRole("heading", { name: "..." })` which uses exact accessible-name matching. Alternatively pass a `RegExp` or `{ exact: true }`.
- **Saturn ring tilt axis controls ellipse orientation (X = landscape, Y = portrait)**: The ring parent's `rotateX(tilt)` produces a wide ellipse (vertical compression via `cos(tilt)`); `rotateY(tilt)` produces a tall ellipse (horizontal compression). The chip counter-rotation must match the same axis (`rotateX(-tilt)` vs `rotateY(-tilt)`) or chips appear skewed. The depth factor for the scale/opacity fade flips too: `sin(θ)` for X-tilt (top comes forward), `−cos(θ)` for Y-tilt (left comes forward). Use `tiltAxis: "y"` in `MOBILE_RING_CONFIGS` so a 235-radius ring becomes ~237×413 on mobile instead of 413×237, matching portrait viewports.
- **`min-h-dvh flex justify-center` on a section wastes 30%+ of mobile height**: A `min-h-dvh` section centering a `h-[70vh]` child bakes in ~15vh of empty space on each side of the child. On mobile, drop `min-h-dvh` and let the section size to content with modest padding (`py-6`) — saves ~240px on typical phones and prevents the "empty black space above/below the ring" look users report.
- **Playwright for screenshots when Chrome MCP can't**: When `mcp__claude-in-chrome__computer action=screenshot` returns an all-black PNG on a narrow viewport (< ~400px wide with absolutely-positioned content), put a one-off `*.spec.ts` in `frontend/e2e/` with `test.use({ viewport: { width: 375, height: 812 } })` and `page.screenshot({ path: "/tmp/xxx.png" })`. Playwright controls viewport independently of the host Chrome window and produces working screenshots. Delete the temp spec before committing.
- **Chrome MCP `resize_window` may no-op when host window can't match**: Requesting 500×900 on a display that constrains the window returns "Successfully resized" but `innerWidth`/`innerHeight` stay unchanged. Always verify the effective viewport via `javascript_tool` after a resize, and fall back to Playwright for viewport-sensitive validation.
- **GDPR/PECR consent gating for Vercel Analytics**: `@vercel/analytics/react` and `@vercel/speed-insights/react` start loading as soon as the component mounts. To gate them on consent without shipping the modules to everyone, wrap each in `React.lazy(() => import("@vercel/analytics/react").then(m => ({ default: m.Analytics })))` and render only when `consent.statistics === true`. React.lazy requires a default export; adapt the named export inside the factory.
- **`window.va` / `window.vaq` as a portable "analytics mounted" signal**: The Vercel Analytics React component calls `inject()` which sets `window.va`, `window.vaq`, `window.vai`, `window.vam` regardless of mode (auto/development/production). Tests that need to assert analytics actually mounted should check `window.va !== undefined` — portable across Vite dev and built binary. URL-based detection is fragile because dev module URLs (`@vercel_analytics`) differ from prod chunk hashes.
- **Radix Dialog modal sets `aria-hidden` on sibling content**: When a Radix `Dialog` is open, the rest of the page is hidden from the a11y tree. Playwright's `getByRole` respects `aria-hidden`, so `getByRole("heading", { name: "Privacy" })` fails to find background-route headings while a modal is open. Use `page.locator("h1", { hasText: "Privacy" })` for assertions that must pass through the modal overlay, or dismiss the modal first.
- **`context.addInitScript` to clear storage fires on every navigation**: Using `page.context().addInitScript(() => localStorage.clear())` in `beforeEach` resets localStorage on every subsequent `page.goto()`, which defeats consent-persistence tests. Playwright already creates a fresh browser context per test by default — just don't add the init script.
- **Versioned consent + shape validation**: Store consent as `{ version: 1, necessary: true, statistics: boolean, updatedAt: ISO }`. On load, shape-validate each field (`typeof`) and return `null` (= re-prompt) on mismatch or version drift. Bumping the version is the mechanism for forcing a re-consent when scope changes.
- **Split context from provider file for react-refresh**: When `react-refresh/only-export-components` is on, a file exporting both `createContext()` and a provider component trips the rule. Put the context + hook in `lib/consent-context.ts` (pure) and the provider in `lib/consent.tsx`. Mirrors the existing `hooks/auth-context.ts` + `hooks/use-auth.tsx` split.
- **Repo list changes touch many assets**: Adding/removing entries in `frontend/src/lib/repos.ts` requires parallel updates in `api/og/index.tsx` (REPO_COUNT), `frontend/index.html` (meta description), `frontend/scripts/og-image-gen.html` (subtitle), `frontend/public/sitemap.xml` (lastmod), `README.md` (category tables), and `frontend/public/og-image.png` (regenerated via Playwright from og-image-gen.html). `MAINTAINERS.md` enumerates the full checklist. A Vitest test in `repos.test.ts` enforces `REPO_COUNT === REPOSITORIES.length`.
- **`FALLBACK_COMBINED_STARS` is arbitrary, not authoritative**: The live combined-stars count displayed on the hero is fetched from GitHub by `useRepoMeta`; the fallback in `home.tsx` is only shown briefly on cold start. If fallback > live value, users see a visible downward flicker when live data loads. Keep fallback at or below a conservative floor of the real combined sum.
- **Saturn ring slice distribution is radius-weighted across a fixed 4 rings**: Ring membership is decoupled from categories via `distributeRepos` + `sortReposForDistribution` in `components/saturn-carousel/distribute-repos.ts`. The outermost ring absorbs rounding remainder so total count is preserved exactly. `RING_SLICES` is computed once at module scope from `DESKTOP_RADII`; mobile reuses the same slice lengths — keep `MOBILE_RADII` proportional to `DESKTOP_RADII` or mobile chip density will silently diverge.
- **Radix `AvatarImage` defeats native `loading="lazy"`**: Radix UI's `AvatarPrimitive.Image` uses `new window.Image()` to preload the avatar before rendering the actual `<img>` tag, so any `loading="lazy"` you pass through is ignored — the browser fetches eagerly via the preload. To get real native lazy loading on hundreds of below-the-fold avatars, render a plain `<img loading="lazy" decoding="async" srcSet="...40 1x, ...80 2x">` yourself with manual onLoad/onError state. Keep the Radix `<Avatar>` Root wrapper for styling; replace `<AvatarImage>` and `<AvatarFallback>` with custom markup. Wrap the new sub-component in `React.memo` since `owner` is a stable prop — otherwise per-card hooks rerun on every parent re-render across ~770 marquee instances.
